...
class Runner(object):
    def __init__(self, meta_agent_id):
        ...
        self.network = PolicyNet(NODE_INPUT_DIM, EMBEDDING_DIM, NUM_ANGLES_BIN, use_trajectory=True, gated_attention=GATED_ATTENTION)
    ...
    def do_job(self, episode_number, curriculum_success_rate=0.0):
    ...
    def job(self, weights_set, episode_number, curriculum_success_rate=0.0):
        print("Starting episode {} on metaAgent {}".format(episode_number, self.meta_agent_id))
        self.set_policy_net_weights(weights_set[0])

        job_results, metrics = self.do_job(episode_number, curriculum_success_rate)
